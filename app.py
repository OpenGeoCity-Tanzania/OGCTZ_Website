...
_db_url = os.environ.get("DATABASE_URL", f"sqlite:///{os.path.join(current_dir, 'data', 'opengeocity.db').replace(os.sep, '/')}")
# SQLAlchemy needs a driver dialect for MariaDB/MySQL. Normalize common shorthand URLs.
if _db_url.startswith("mariadb://"):
    _db_url = _db_url.replace("mariadb://", "mariadb+pymysql://", 1)
...
def inject_global_vars():
    ...
    return {
        ...
        "latest_posts": lambda limit=3: published_posts_query(limit=limit).all()
    }
...
# ── CMS Routes ──────────────────────────────────────────────────────────────
# Admin and public blog routes are registered as blueprints from the cms package.
